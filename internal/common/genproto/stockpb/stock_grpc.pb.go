...
const (
	...
	StockService_CheckItemsInStock_FullMethodName = "/stockpb.StockService/CheckItemsInStock"
)
...
type StockServiceClient interface {
	...
	CheckItemsInStock(ctx context.Context, in *CheckItemsInStockRequest, opts ...grpc.CallOption) (*CheckItemsInStockResponse, error)
}
...
func (c *stockServiceClient) CheckItemsInStock(ctx context.Context, in *CheckItemsInStockRequest, opts ...grpc.CallOption) (*CheckItemsInStockResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(CheckItemsInStockResponse)
	err := c.cc.Invoke(ctx, StockService_CheckItemsInStock_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func _StockService_CheckItemsInStock_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(CheckItemsInStockRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(StockServiceServer).CheckItemsInStock(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: StockService_CheckItemsInStock_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(StockServiceServer).CheckItemsInStock(ctx, req.(*CheckItemsInStockRequest))
	}
	return interceptor(ctx, in, info, handler)
}
...
var StockService_ServiceDesc = grpc.ServiceDesc{
	...
	Methods: []grpc.MethodDesc{
		...
		{
			MethodName: "CheckItemsInStock",
			Handler:    _StockService_CheckItemsInStock_Handler,
		},
	},
